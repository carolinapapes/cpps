...
void upperBoundDebug(const std::deque<MultiLevelPairsDq>& main, int x, int y)
{
	...
	printNodes(main, "\n\n\tCURRENT");
	std::cout << RESET << GRAY;
	std::cout << "\tLEAD VAL " << x  << std::endl;
	std::cout << "\tPAIR IDX " << y - 1 << std::endl;
	std::cout << "\n";
	std::cout << RESET;
}
...
static std::vector<size_t> jacobOrder(size_t nodesQty)
{
	...
	// if (VERBOSE)
	// 	printInsertionOrder(order);
	return order;
}

static size_t upperBoundLead(const std::deque<MultiLevelPairsDq>& main,
                                  int x,
                                  size_t y)
{
